    ...
    <div class="row align-items-center g-5">
        <div class="col-lg-6">
            ...
            <div class="d-flex gap-2">
                @auth
                    <a href="{{ route('dashboard') }}" class="btn btn-primary btn-lg px-4">Buka Dashboard</a>
                @else
                    <a href="{{ route('register') }}" class="btn btn-primary btn-lg px-4">Mulai Sekarang</a>
            ...
            <div class="row g-3">
                ...
                <div class="col-6">
                    <div class="card stat-card p-4 h-100">
                        ...
                        <div class="fs-3 fw-bold">{{ $totalJenis }}</div>
        ...
        @forelse ($featuredProducts as $product)
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 shadow-sm">
                    <img src="{{ $product->imageUrl() }}" class="product-thumb" alt="{{ $product->nama_barang }}">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start">
                            <h5 class="card-title mb-1">{{ $product->nama_barang }}</h5>
                            <span class="badge text-bg-success">Tersedia</span>
                        </div>
                        <p class="text-secondary small mb-2">{{ $product->category->name }} &middot; {{ $product->kode_barang }}</p>
                        <div class="d-flex justify-content-between small text-secondary">
                            <span><i class="bi bi-geo-alt me-1"></i>{{ $product->lokasi_penyimpanan ?? '-' }}</span>
                            <span><i class="bi bi-boxes me-1"></i>Stok: {{ $product->stok }}</span>
                        </div>
                    </div>
                </div>
            </div>
